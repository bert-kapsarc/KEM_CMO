...
;
end_day(seasons)$winter(seasons) = start_day('spfa')-1;
...
number_of_days(seasons)$spring(seasons) = number_of_days('spfa')+end_day_fall-start_day_fall+1;
...
sum(hrs$(
         ...
         hour(hrs)>=block_start(seasons,l) and
         hour(hrs)<block_end(seasons,l)
  ), (HLC(r,hrs)-ELlcgw(r,seasons,l))*(HLC(r,hrs)-ELlcgw(r,seasons,l)) )/(duration(seasons,l)-1)
;
